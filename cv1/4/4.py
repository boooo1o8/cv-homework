...
def replicate_padding(image_array, padding_size):
    ...
    for i in range(padding_size):
        padded_image = np.insert(
            padded_image, 0, padded_image[0], axis=0)
        padded_image = np.insert(
            padded_image, height + 1, padded_image[height], axis=0)
        padded_image = np.insert(
            padded_image, 0, padded_image[:, 0], axis=1)
        padded_image = np.insert(
            padded_image, width + 1, padded_image[:, width], axis=1)
...
def show(image, padding_size):
    ...
    imageio.imwrite(os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'padded_image_zero.jpg'), padded_image_zero)
    imageio.imwrite(os.path.join(os.path.dirname(
        os.path.abspath(__file__)), 'padded_image_replicate.jpg'), padded_image_replicate)
    plt.show()
